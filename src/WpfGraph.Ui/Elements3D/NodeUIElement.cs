...
namespace Palmmedia.WpfGraph.UI.Elements3D
{
    ...
    internal class NodeUIElement<TNodeType, TEdgeType> : GraphUIElement
        ...
        where TEdgeType : EdgeData
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="NodeUIElement&lt;TNodeType, TEdgeType&gt;"/> class.
        /// </summary>
        /// <param name="node">The node.</param>
        public NodeUIElement(Node<TNodeType, TEdgeType> node)
            : base(node.Data)
        {
            this.Node = node;
        ...
        /// Gets the node.
        /// </summary>
        protected Node<TNodeType, TEdgeType> Node { get; private set; }
        ...
        protected override void OnMouseLeftButtonDown(System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            var graphProvider = IocHelper.Resolve<IGraphProvider<TNodeType, TEdgeType>>();

            var selectedElement = graphProvider.SelectedElement;
            if (selectedElement != null
                && selectedElement is Node<TNodeType, TEdgeType>
                && (DateTime.Now - lastClickTime).TotalSeconds < 1)
            {
                var firstNode = (Node<TNodeType, TEdgeType>)selectedElement;

                var edge = new Edge<TNodeType, TEdgeType>(firstNode, this.Node);
                graphProvider.Graph.Add(edge);
                graphProvider.SelectedElement = edge;
            }
            else
            {
                graphProvider.SelectedElement = this.Node;
            }
