...
namespace Palmmedia.WpfGraph.UI.Windows
{
    ...
    public partial class MainWindow : Window
    {
        ...
        protected override void OnKeyUp(System.Windows.Input.KeyEventArgs e)
        {
            ...
            if (e.Key == System.Windows.Input.Key.W && !(e.OriginalSource is TextBox))
            {
                var graphProvider = IocHelper.Resolve<IGraphProvider<NodeData, EdgeData>>();

                var selectedElement = graphProvider.SelectedElement;

                if (selectedElement != null)
                {
                    if (selectedElement as Edge<NodeData, EdgeData> != null)
                    {
                        graphProvider.Graph.Remove((Edge<NodeData, EdgeData>)selectedElement);
                    }
                    else if (selectedElement is Node<NodeData, EdgeData>)
                    {
                        graphProvider.Graph.Remove((Node<NodeData, EdgeData>)selectedElement);
                    }
                }
            }
        }
        ...
        private void OnViewportMouseDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            if (selectedElement==null) {
                for (int i = 0; i <= 4; i++)
                {
                    for (int k = 0; k <= 4; k++)
                    {
                        var nodeData = new NodeData(new Point3D(point.X + temp * k, point.Y + i * temp, 0));
                        var node = new Node<NodeData, EdgeData>(nodeData);
                        graphProvider.Graph.Add(node);
                        graphProvider.SelectedElement = node;
                    }
                }
            }
            /*point.Y = -point.Y;
           ...
           var nodeData1 = new NodeData(new Point3D(point.X+10, point.Y+10, 0));
           var node1 = new Node<NodeData, EdgeData>(nodeData1);

           var graphProvider1 = IocHelper.Resolve<IGraphProvider<NodeData, EdgeData>>();
           graphProvider1.Graph.Add(node1);
           graphProvider1.SelectedElement = node1;*/
        }
    }
}